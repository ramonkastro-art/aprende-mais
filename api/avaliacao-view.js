...
function normalizarQuestao(q) {
  if (!q || typeof q !== 'object' || !q.alternativas || typeof q.alternativas !== 'object') return null;
  const gabarito = String(q.gabarito || '').trim().toUpperCase();
  if (!LETRAS.includes(gabarito)) return null;
  const enunciado = String(q.enunciado || '').trim();
  if (!enunciado) return null;
  const alternativas = {};
  for (const letra of LETRAS) {
    alternativas[letra] = String(q.alternativas[letra] || '').trim();
    if (!alternativas[letra]) return null;
  }
  return { enunciado, alternativas, gabarito };
}

function validarAvaliacao(parsed, qtd) {
  if (!parsed || !Array.isArray(parsed.questoes) || parsed.questoes.length !== qtd) {
    throw new Error(\`A IA retornou \${parsed?.questoes?.length || 0} questões; eram esperadas \${qtd}. Tente novamente.\`);
  }
  const questoes = parsed.questoes.map(normalizarQuestao);
  if (questoes.some(q => !q)) throw new Error('Uma ou mais questões vieram incompletas. Tente novamente.');
  return questoes;
}

function enfileirarSalvamentoAtual() {
  if (!avaliacaoAtual) return Promise.resolve();
  const anterior = salvamentoPendente || Promise.resolve();
  salvamentoPendente = anterior
    .catch(() => {})
    .then(() => salvarAvaliacao(avaliacaoAtual.questoes, avaliacaoAtual.config));
  return salvamentoPendente;
}

function agendarSalvamento() {
  clearTimeout(salvamentoTimer);
  salvamentoTimer = setTimeout(() => {
    salvamentoTimer = null;
    enfileirarSalvamentoAtual();
  }, 700);
}
...
async function gerarAvaliacao() {
  ...
  try {
   const systemPrompt = \`Você é um professor brasileiro experiente em avaliação da Educação Básica, com domínio de elaboração de itens objetivos, inclusão e habilidades da BNCC. Produza uma avaliação pronta para revisão do professor.

TAREFA: Gere exatamente \${qtd} questões de múltipla escolha para \${comp}, \${ano}, nível \${nivel}, usando exclusivamente o conteúdo/tópicos informado pelo professor.
...
- O enunciado deve ser claro, completo, objetivo e adequado à idade do \${ano}; não dependa de imagem, tabela ou informação externa que não esteja no JSON.
...
Gere exatamente \${qtd} questões seguindo as regras. Responda APENAS com o JSON, nenhum texto adicional.\`;
    ...
    if (!res.ok || data.error) {
      if (data.code === 'AI_LIMIT') throw new Error('A IA está temporariamente indisponível. Tente novamente mais tarde.');
      throw new Error(data.error || 'Não foi possível gerar a avaliação. Tente novamente.');
    }
    ...
    const questoesValidadas = validarAvaliacao(parsed, parseInt(qtd, 10));
    ...
    salvamentoPendente = salvarAvaliacao(avaliacaoAtual.questoes, avaliacaoAtual.config);
  ...
  avaliacaoAtual.questoes.forEach((q, qi) => {
    ...
    const altsHTML = LETRAS.map(l => \`
      <div class="alt-row">
        ...
        <input class="alt-input" 
               value="\${escapeHtml(q.alternativas[l] || '')}" 
               oninput="avaliacaoAtual.questoes[\${qi}].alternativas['\${l}']=this.value; agendarSalvamento()"
    ...
    card.innerHTML = \`
      ...
      <textarea class="questao-enunciado" rows="3"
        oninput="avaliacaoAtual.questoes[\${qi}].enunciado=this.value; agendarSalvamento()"
...
function setupCorrecao() {
  ...
  document.getElementById('infoAvaliacao').innerHTML = \`
    <strong>\${escapeHtml(c.comp)}</strong> · \${escapeHtml(c.ano)}\${c.turma ? ' · '+escapeHtml(c.turma):''} · \${c.qtd} questões<br>
    \${escapeHtml(c.conteudo)}
  \`;

  if (respostasAluno.length !== avaliacaoAtual.questoes.length) {
...
function verificarCompleto() {
  ...
  else document.getElementById('resultadoBox').style.display = 'none';
}
...
function calcularNota() {
  const total = avaliacaoAtual?.questoes?.length || 0;
  if (!total) { document.getElementById('notaFinal').textContent = '0,0'; return; }
  const acertos = avaliacaoAtual.questoes.filter((q,i) => respostasAluno[i] === q.gabarito).length;
  const campo = document.getElementById('valorProva');
  const valor = Math.min(100, Math.max(1, parseFloat(campo.value) || 10));
  campo.value = valor;
...
async function salvarAvaliacao(questoes, config) {
  const versao = avaliacaoVersao;
  const gabStr = questoes.map(q => q.gabarito).join('');
  const localId = 'LOCAL-' + gabStr;
  if (!avaliacaoId || !/^AV-\\d{4}-[A-Z0-9]{4,8}$/.test(avaliacaoId)) avaliacaoId = localId;

  const badge = document.getElementById('avIdBadge');
  if (badge) { badge.textContent = '🔑 Gabarito pronto'; badge.style.display = 'inline-flex'; }

  try {
    const persistId = /^AV-\\d{4}-[A-Z0-9]{4,8}$/.test(avaliacaoId) ? avaliacaoId : null;
    const res = await fetch('/api/avaliacao-salvar', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ id: persistId, questoes, config })
    });
    const data = await res.json().catch(() => ({}));
    if (data.id && versao === avaliacaoVersao) {
      avaliacaoId = data.id;
    ...
    console.warn('Persistência indisponível, modo local ativo:', e.message);
  }
}
...
async function exportarPDF() {
  ...
  // Enfileira imediatamente uma edição recente que ainda aguardava o debounce.
  if (salvamentoTimer) {
    clearTimeout(salvamentoTimer);
    salvamentoTimer = null;
    enfileirarSalvamentoAtual();
  }
  // Aguarda uma gravação pendente, mas preserva a exportação local se o banco estiver indisponível.
  if (salvamentoPendente) {
    try { await salvamentoPendente; } catch (e) { console.warn('PDF usando modo local:', e.message); }
  ...
  for (let qi = 0; qi < questoes.length; qi++) {
    ...
    doc.setFontSize(11);
    const enuncLines = doc.splitTextToSize(\`\${qi+1}. \${q.enunciado}\`, cW);
  ...
  {
    try {
      const qrUrl = \`\${window.location.origin}/corrigir?id=\${encodeURIComponent(qrIdParaUsar)}\`;
  ...
  const nomeSeguro = value => String(value || '').replace(/[^a-zA-Z0-9À-ÿ]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 80) || 'Avaliacao';
  doc.save(\`\${nomeSeguro(\`Avaliacao_\${c.comp}_\${c.ano}_\${c.geradaEm}\`)}.pdf\`);
